...
def exit_options(room):
    options = []
    for option in room.getExits():
        if rooms[room.id][option] == '?':
...
# check exit options from each path in current room
# check options[0]. If None, pop from options and add to
# visited

def breadth_first_search(room):
    visited = {}
    options = []
    # get the options in gthe current room
    for option in room.getExits():
        options.append(option)
    # save the first option in visited
    if room.id in visited:
        visited[room.id] + options[0]
    else:
        visited[room.id] = options[0]
    # while there are options..
    while len(options) > 0:
        # save first option, move
        temp_option = options.pop(0)
        player.travel(temp_option)
        # if options is empty and the new room has no options..
        # **** prob is that we go back to a room with no options. If options is empty, we should stay in new room hunting
        if len(options) == 0 and exit_options(player.currentRoom) == None:
            for option in room.getExits():
                options.append(option)
                print("Options..", options, room.id, rooms)
        # move to new room, push options to options
        # if there's nothing in the new room, go back
        elif exit_options(player.currentRoom) == None:
            player.travel(reverse[temp_option])
        else:
            options = []


def traverse(rooms, roomGraph):
    completedPaths = False
    while completedPaths == False:
        if player.currentRoom.id not in rooms:
            add_new_room(player.currentRoom)
        else:
            # print("Options..", exit_options(player.currentRoom))
            if exit_options(player.currentRoom) == None:
                breadth_first_search(player.currentRoom)
            else:
                direction = exit_options(player.currentRoom)
                print("Test", direction, player.currentRoom.id, rooms)
                currentRoom = player.currentRoom.id
                player.travel(direction)
                traversalPath.append(direction)
                newRoom = player.currentRoom
                print("Reverings", direction, reverse[direction])
                rooms[currentRoom][direction] = newRoom.id
                if newRoom not in rooms:
                    add_new_room(newRoom)
                rooms[newRoom.id][reverse[direction]] = currentRoom
                print('Rooms', rooms)
            # for option in player.currentRoom.getExits():

            # currentRoomOption = traversalPath[player.currentRoom.id][option]
...
print(traverse(rooms, roomGraph))


# print("ID", player.currentRoom.id)
# print("Get Exists", player.currentRoom.getExits())
# print("ID", player.travel('n'))
# print("World", world.roomGrid)
# print("ID", player.currentRoom.id)
...
# def traverse(traversalPath, roomGraph):
#     # queue = player.currentRoom.getExits()
#     options = ['n', 's', 'e', 'w']
#     test = True
#     while test == True:
#         print("******", traversalPath)
#         # go through each option in options
#         for option in range(len(options)):
#             currentRoom = player.currentRoom.id
#             # check if all of the directions are known
#             if '?' not in traversalPath[player.currentRoom.id].values():
#                 # if they are all known, and the current room is the last room,
#                 # make the current room 0
#                 # *** we could do a while loop here to break out of the outer
#                 # loop if all values in graph are known
#                 if currentRoom == len(roomGraph)-1:
#                     player.currentRoom.id = 0
#                     currentRoom = 0
#                 # else increase by 1
#                 else:
#                     player.currentRoom.id += 1
#                     currentRoom = player.currentRoom.id
#                     # if the current room isn't in the traversal
#                     if currentRoom not in traversalPath:
#                         temp = {}
#                         for item in player.currentRoom.getExits():
#                             temp[item] = '?'
#                             traversalPath[player.currentRoom.id] = temp
#             elif options[option] in traversalPath[currentRoom]:
#                 # check if the option available is an unknown ? or is known
#                 # if the direction in the room is known, continue to the next
#                 # option in the room **** could check if the option is pos
#                 if traversalPath[currentRoom][options[option]] is '?':
#                     # if the direction in the room is unknown..
#                     # go in that direction
#                     player.travel(options[option])
#                     # save next room id
#                     nextRoom = player.currentRoom.id
#                     # update old room with the path to the next room
#                     traversalPath[currentRoom][options[option]] = nextRoom
#                     temp = {}
#                     # if we already have the next room id in travPath
#                     # save temp as the path we have already have
#                     # *** what are we doing with this ***
#                     if nextRoom in traversalPath:
#                         temp = traversalPath[nextRoom]
#                     # if it's a new room, save the exit options and
#                     # add a ? as all options are unknown
#                     else:
#                         for item in player.currentRoom.getExits():
#                             temp[item] = '?'
#                             traversalPath[player.currentRoom.id] = temp
...
# def helper(start, traversalPath):
#     completeRow = False
#     player.currentRoom.id = start
#     print("Starting =====>", player.currentRoom.id)
#     while completeRow == False:
#         completeRow = True
#         if player.currentRoom.id not in traversalPath:
#             temp = {}
#             for item in player.currentRoom.getExits():
#                 temp[item] = '?'
#                 traversalPath[player.currentRoom.id] = temp
#         for option in traversalPath[start]:
#             if traversalPath[start][option] == '?':
#                 completeRow = False
#         for exit in player.currentRoom.getExits():
#             currentRoomExits = player.currentRoom.getExits()
#             print("Swing low, sweet =>", player.currentRoom.id, exit,
#                   traversalPath[player.currentRoom.id][exit])
#             print(traversalPath)
#             if exit not in player.currentRoom.getExits():
#                 continue
#             elif traversalPath[player.currentRoom.id][exit] != '?':
#                 print('Current for', exit, player.currentRoom.id)
#                 continue
#             else:
#                 currentRoom = player.currentRoom.id
#                 player.travel(exit)
...
#                 nextRoom = player.currentRoom.id
#                 temp = {}
#                 traversalPath[currentRoom][exit] = nextRoom
#                 print(traversalPath)
#                 if nextRoom in traversalPath:
#                     temp = traversalPath
#                 else:
#                     for item in currentRoomExits:
#                         temp[item] = '?'
#                         traversalPath[player.currentRoom.id] = temp
...
#         elif option == 'w':
#             reverse = 'e'
#         if traversalPath[player.currentRoom.id][option] == '?':
#             currentRoom = player.currentRoom.id
#             player.travel(option)
#             nextRoom = player.currentRoom.id
#             traversalPath[currentRoom][option] = nextRoom
#             print("Current id", player.currentRoom.id)
#             player.currentRoom.id = start
#             player.travel(reverse)
#             print("Next id", player.currentRoom.id)
#     return traversalPath

# create bfs
# while loop with boolean
# check if room is full
# if it is full, check [0]
# if that rooms full, move back and check the rest of the options in the room
# if all other options are full, move to next room and try the same
# save each move in array. if array is the same size as the graph, break from loop
