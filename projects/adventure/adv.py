...
def add_new_room(room):
    ...
    for exit in room.getExits():
        temp[exit] = "?"
        visited[room.id] = temp


while len(visited) < len(roomGraph):
    options = {}

    # if the current room isn't in room, add it
    if player.currentRoom.id not in visited:
        add_new_room(player.currentRoom)

    # Get available exit in the current room
    options = visited[player.currentRoom.id]
    open_paths = False
    for direction, exit_room in options.items():
        
        # if there's an unknown exit, go there and add the new room
        if exit_room == '?':
            current_room = player.currentRoom.id
            player.travel(direction)
            traversalPath.append(direction)
            options[direction] = player.currentRoom.id

            # check if the new room has been visited
            if options[direction] not in visited:
                add_new_room(player.currentRoom)

            # if new room has been visited, go backwards and get rid of the old direction from traversalPath
            else:
                open_paths = True
            ...
            # Push reverse direction to queue so we can travel back if necessary
            queue.append(reverse[direction])

            open_paths = True  
            break

    # already visited the room so we pop off the queue
    if open_paths == False:  
        move_back = queue.pop()

        # if we're back at the beginning, break
        if move_back == None:
            break
